...
- Android 客户端：VLESS Reality 导入、订阅拉取、剪贴板导入、Xray outbound JSON 导出、系统 `VpnService` 权限申请、VPN 启动/停止入口、私有 Xray + tun2socks runtime 注入、debug/release unsigned APK 和可选 signed APK/AAB
- iOS 客户端：VLESS Reality 导入、订阅拉取、剪贴板导入、Xray outbound JSON 导出、NetworkExtension 配置管理、嵌入式 PacketTunnel target、私有 PacketCore runner 接口、simulator 构建和可选 signed IPA
